...
class NetEase(LRCProvider):
    ...
    def __init__(self, need_translation: bool) -> None:
        super().__init__()
    ...
    def get_lrc_by_id(self, track_id: str) -> Optional[Lyrics]:
        params = {
            "id": track_id,
            "lv": 1,
        }  # The api has lv for lyrics, tv for translation and kv for unknwon
        if self.need_translation:
            params["tv"] = 1
        response = self.session.get(self.API_ENDPOINT_LYRICS, params=params)
        lrc = Lyrics()
        response = response.json()
        lrc.add_unknown(response.get("lrc", {}).get("lyric"))
        if self.need_translation:
            lrc.merge_translation(response.get("tlyric", {}).get("lyric"))
        return lrc
